...
**Cloud Tasks migration (2026-08-13/15) — BackgroundTasks is EXTINCT in this codebase.** All three long-running work types are durable jobs executed as authenticated HTTP POSTs back to this service (CPU guaranteed inside the request; prod Cloud Run throttles CPU post-response, which is why BackgroundTasks kept losing work). One substrate, three kinds (`cloud_tasks_service.JobKind`): extraction (`rubric_extraction_jobs`, PR-1), **batch transcription** (`transcription_jobs`, migration 016), **grading** (`graded_tests` rows — already row-first; only the substrate changed). Do not add new work on BackgroundTasks — declare a JobKind.
- **Batch transcription:** `create_batch` uploads every PDF to GCS (all-or-nothing 502 on failure, owner-ratified), commits batch + one `TranscriptionJob` per doc in ONE transaction (`Σ jobs == test_count` ALWAYS — "in flight" is a fact, never inference over absence; the phantom-item class is structurally dead), then enqueues one task per job. Worker (`transcription_job_runner`): CAS `queued→running` → GCS download → pipeline (untouched) → **ONE transaction: transcriptions INSERT + job→completed** (zombie completions roll back, never double). Failure → `failed` + error + net_diag verdict on the row. Per-doc **retry without re-upload**: `POST /batches/{id}/jobs/{job_id}/retry` (failed or expired only, CAS).
...
**Production grader pin (OWNER VERDICT 2026-08-31): `claude-sonnet-5` + `grader-v5.3` + plan `hobby_tvshow`.** Supersedes gemini-3.1-pro, which failed the cost gate on 10/10 measured trials ($0.3823 vs the $0.15 ceiling) and graded 3.4x slower. **The verdict knowingly accepts a K1 failure** (45/48, all three firings the single carried cell `din/q2.ב.c4.s2`, which the previous champion also leaks 1/3 of the time) — the bar was not moved, the failure is accepted with its name attached. Model and prompt are a PACKAGE; v6 is a dated artifact, not a pin. ⚠ **The pin is DARK until a rubric is bound**: `grader_architecture` stays `v3` and `grader_plan_rubric_id` stays unset because no production rubric has a ratified plan (6 rubrics, `graded_tests = 0`, checked 2026-08-31). Setting v5 without the binding logs `grader_pin_incomplete` on every grade and falls back to v3 anyway. Activation is `GRADER_PLAN_RUBRIC_ID=<uuid>` + `GRADER_ARCHITECTURE=v5`. The ratified plan ships at `app/agents/grader/plans/` — **not** under `tests/`, because the Dockerfile copies `app/` and nothing else, so a tests/ path silently resolves to the v3 fallback.
...
| `auth.py` / `users.py` | `/api/v0/auth`, `/api/v0/users` | auth (login/signup/**profile**/logout/refresh) · users: subject matters, the user's rubric + graded-test lists, rubric sharing, and the **onboarding writes** (`PUT /me/schools`, `PATCH /me/profile` — the ONLY writer of `full_name`, `POST /me/onboarding/complete`; none mounts at `/users/me`, which would turn its 404 guard into a 405). **The profile read lives at `GET /auth/me` and only there** — `users.py` carried a caller-less byte-identical duplicate at `GET /users/me`, deleted 2026-08-17 |
...
- `app/services/transcription_job_runner.py` — `run_transcription_job(job_id)`: CAS `queued→running` → GCS download → pipeline (untouched) → ONE transaction (transcriptions INSERT + job→completed; zombie completions rolled back); heartbeat sidecar (~60s); bounded doc re-run on retryable transport failure; failures land on the row with the net_diag verdict. Runs inside `/internal/transcription-jobs/{id}/run`.
...
- `transcription_job.py` — `TranscriptionJob` (Cloud Tasks migration, migration 016): one durable row per batch PDF, created in the same commit as the batch (`Σ jobs == test_count` always); CHECK `transcription_jobs_status_consistency`; `grading_batches.transcription_failures` (015 ledger) is DORMANT — legacy read-only, dropping it is B-27.
...
| `src/app/onboarding/` + `src/components/onboarding/` | **The onboarding flow (migration 022)** — five steps in one non-dismissible centered dialog (welcome → subjects → schools → name+gender → ready), on the `batch/Modal` primitive (`dismissible={false}`, `size="lg"`; both props default to the old behavior). Each step COMMITS when she advances past it, so a drop-off keeps her answers; `POST /me/onboarding/complete` stamps the end. `components/OnboardingGate.tsx` (mounted in the root layout) redirects any authenticated teacher whose `onboarding_completed_at` is null. ⚠️ **Every e2e user fixture must carry that stamp** or the gate redirects the whole Playwright suite — `e2e/fixtures.ts::USER` and `e2e/seedBatch.ts::AUTH_ME` do |
