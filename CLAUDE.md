...
| `classroom.py` | `/api/v0/classroom` | students + classes + memberships. **`GET /students/{id}/signed-tests`** (student-profile PR, Part A): every APPROVED test of a student, one row per chain `(transcription_id, rubric_id)`, newest upload first — named for the invariant it carries (M-A1: a future «add drafts» is a visible rename). The definition lives in ONE place, `services/student_signed_tests.py::signed_leaves` (LST-1..4); the roster's `signed_tests_count` is one grouped statement over it, never a query per student (pinned by a statement count). `students.notes` is GONE (migration 031, OD-3) — a body still sending it is ignored. `DELETE /students/{id}` is the **privacy-complete purge** (Part B): 200 with the verify report (M-B4), 409 `grading_in_progress` while a grade or job of hers is in flight (PRV-5), 409 `purge_refused` with a reason code when the plan cannot promise a correct purge; `GET /students/{id}/purge-preview` is the plan, counted, for the Delete dialog. Both go through `services/erasure` and its ONE storage dependency `get_purge_storage` — the thing tests fake (a session-wide guarded fake in `tests/api/conftest.py`), never auth. |
...
- **`app/services/erasure/` — the student purge (Part B; `docs/PURGE_CENSUS.md` is its spec).** `plan.py` (read-only, the WHOLE discovery: her scans' subtrees + memberships + the student row; objects found by LISTING every known bucket; refuses on `selection_mismatch` / `dead_table_rows` / `unknown_bucket` / `unsafe_target` / `shared_object` before any storage call, naming rows by id, never a path), `execute.py` (`purge_student`: FOR UPDATE on the student, plan inside the lock, rows children-first with the student row LAST in one transaction, then objects, then verify and one audit line — ids and counts only), `verify.py`, `ledger.py` (`purge_failures`, migration 033), `targets.py` + `storage.py` (**PRV-11: exactly three prefix families, and every storage call validated before the client sees it**), `registry.py` (every caught column classified; set-compared with the live schema by a test). ⚠ **Never delete a student row and let the constraints do the rest** (ruling 2026-09-22): since 032 they refuse, and that refusal is PRV-8 working. Commands: `python -m app.scripts.retry_purge_failures`, `python -m app.scripts.verify_purge <student_id> --user <id>`.
...
Raw SQL, zero-padded, sequential (`NNN_description.sql`). No Alembic. Current head: **033**. The schema is created/reshaped by 007 (cleanup) + 008 (new schema); 009 (graded-test cost columns); 010 (revision-chain deferrable FK); 011 (S11 transcription `batch_id` + batch `test_count`); 012 (`rubric_extraction_jobs` + `rubrics.extraction_job_id`); 013 (`schema_migrations` ledger + commit-token convention); 014 (`transcriptions.review_json`); 015 (`grading_batches.transcription_failures` ledger — dormant since 016); 016 (`transcription_jobs` — the Cloud Tasks batch fan-out); 017 (`transcription_jobs.client_file_id`); 018 (`schools` + `users.school_id`, PR-G6); 019 (`graded_tests.opened_at`, PR-G8); 020 (`grading_batches.appendix_include_criteria` + `stamp_position_default`, PR-G9); 021 (`graded_tests.returned_exam_key`, PR-G9); 022 (onboarding: `users.gender` + `users.onboarding_completed_at` + the ordered `user_schools` junction); 023 (`schools.ministry_symbol` + its partial unique index; 018's whole-table name index narrowed to the symbol-less rows); 024 (auth: `users.email_verified_at` + `email_verification_codes` + `auth_nonces`); 025 (`grading_batches.expected_test_count` — the upload-stage in-flight fact); 026 (`grading_plans` — compiled GradingPlans keyed by contract hash, PLAN COMPILER v2 production wiring); 027 (`rubrics.subject` — the multi-subject seam: NOT NULL, server-default `computer_science`, backfilled from the contract/draft JSON, indexed); 028 (the onboarding next-exam step + outreach queue: `users.next_exam_date` / `next_exam_answered_at` / `next_exam_reask_email_sent_at` / `phone` / `whatsapp_opt_in` / `guided_session_requested_at`, plus the partial index `idx_users_reask_candidates`); 029 (`graded_tests.transcription_contract_version` — VER-3, the OTHER half of the grading input; NULLABLE and never back-filled, because a guessed provenance is worse than a recorded absence); 030 (the last ten NAIVE timestamp columns → `TIMESTAMPTZ`. The ORM declared `DateTime(timezone=True)` and the DB disagreed, so the driver returned a naive datetime and `.isoformat()` emitted NO OFFSET — which ECMAScript parses as LOCAL time, and the batch-completion hero read «181 דקות» for a batch created 4 minutes earlier. Migration 022's lesson from the other direction); 031 (`students.notes` DROPPED — student-profile PR OD-3/M-A6; production held 0 non-null notes across 9 students on 2026-09-19, so the column went rather than the reads alone); 032 (every FK OUT OF the five student-data tables → ON DELETE NO ACTION — AM-B1: 18 FKs dropped and re-created under their names in one transaction, `regraded_to_id` still DEFERRABLE INITIALLY DEFERRED, `class_memberships` keeps its cascades — plus AM-B4's UNIQUE (id, user_id) and four `*_tenant_fkey`. GENERATED by `backend/scripts/gen_migration_032.py`; its `--check` against production is the deploy gate, preceded by census query D and a production `pg_dump` to `db-archive/`, PITR being off. After 032 a rubric, user or batch delete FAILS while student data references it — that is the design); 033 (`purge_failures`, the purge's ledger). **PRODUCTION IS AT 033** (031–033 applied 2026-09-23 after a `pg_dump` to `db-archive/vivi_prod_pre032_20260923_125221.dump` — PITR is off, so that dump is the restore point; the boot check reads `SCHEMA OK: migration head 033`). ⚠ **Vivi-Test is at 031 + 033 WITHOUT 032**: six cross-tenant graded tests planted by Part A's old leak test (s2test.com users) block AM-B4's tenant FK, and 032 rolled back whole, as designed; deleting those six rows is Noam's call. Until then Vivi-Test's boot check reports 032 missing (ledger read 2026-09-11: 001–030 contiguous, `rubrics.subject` and the 028 columns present) — the «Vivi-Test only» warnings that stood here for 027/028 are retired, and a session that re-applies them from this doc is reading a stale copy. Caveat: the `rubrics` and `grading_batches` base tables predate migration 001 (created unversioned via `create_all`) — the numbered series only ALTERs them. The ORM mirrors the migrated schema — it does not generate it. When the DB and a schema doc disagree, the DB is truth.
...
>
> ⚠️ **No log line carries a filename or a student's name (OD-B4, 2026-09-22).** Teachers name scans after the student, so a filename IS a name, and Cloud Logging keeps it 30 days where no purge can reach it. Log the job / transcription / graded-test id instead. The transcription pipeline's `doc_id` is that id — the filename travels separately, only into the identity pass, which reads it as a fallback source of the name and never logs it. Pinned by `tests/test_logs_carry_no_student_names.py`, a structural AST scan of `app/` whose few exemptions (dead modules, uncalled parsers, the legacy engine's command-line benchmark) are each re-verified as still dead.
...
| `src/utils/batch-stage.ts` | **THE batch stage — ONE pure function, and the rule is that there is only one** (grading-flow clarity pass, 2026-09-14). `deriveBatchStage(batch) → {step, chip, turnLine}`; the header chip, the turn line and the six-step stepper are three renderings of that single value, on BOTH the detail page and the list. **Never component-local, never a second derivation.** It replaces `copy/batch.ts::batchStatusLabel` and `utils/batch-dashboard.ts::selectHeadline`, which were DELETED rather than repaired (the §9 precedent): the label knew about uploading and transcribing and nothing about the grading half of the rollup, so a batch Vivi was actively grading showed «ממתין להחלטות» — it told the teacher she was holding it up while the machine worked. Pinned row-by-row in `batch-stage.test.ts` (zero mocks), including the stale-chip payload itself. ⚠ `BatchDetailResponse.eta` is the **grading** ETA only; there is no transcription ETA on the wire, so the transcription turn lines carry no «עוד כ-N דקות» clause rather than borrowing the wrong stage's figure |
| `src/components/batch/TranscriptionTriage.tsx` | §5.3C — the transcription gate as a card grid (needs-a-look FIRST, then the confidently-read), replacing `CleanPanel` + `NeedsEyesQueue`. It **states sentences about the server's verdict and never re-triages**: section membership is `assignZones`, the reason lines are `utils/triage-reason.ts`, the clean card's line is `utils/transcription-completeness.ts` (selection-aware: under a «choose k of N» group it reports COUNTS and never names which questions are "missing" — the student chose). Page-1 thumbnails ride `BatchTranscriptionItem.page1_image_url`, minted server-side by the same `thumbnail.page_image_path` the graded pile uses. ⚠ **`deriveCompleteness` REFUSES two shapes rather than computing them** (`not_computable`): a question in two selection groups (§5.3D ruled it undefined — computing it counts one answer toward two requirements), and a group whose `choose_k` exceeds its member count, which `answer_space_groups` PRODUCES BY DESIGN when it drops dangling member ids and leaves `choose_k` alone — unrefused, that renders a permanent «נמצאו 4 שאלות מתוך 5 הנדרשות» on an exam the student completed. Both set `needsLook: false`: the gap is ours, and sending her to a document that is fine is the over-count failure §3.5a names |
| `src/components/batch/StageStepper.tsx` · `FirstBatchExplainer.tsx` · `CompletedStageStrip.tsx` | The stage model's surfaces. **Exactly ONE celebration exists in the flow** and it is the end state (`grade-review/SignedCompletion.tsx`, fired by `signOffReached` — every graded test SIGNED); a finished stage collapses to a one-line strip. `CompletionHero` is deleted: the same green-check card after transcription approval *and* after signing taught her the first one was the end, four steps early. The explainer reads `BatchDetailResponse.is_first_batch` (a server fact, so it cannot reappear on a second device; a BOOLEAN and not an ordinal because the detail endpoint is the 3-second poll target and an ordinal cost a full COUNT per tick for a value only ever compared to 1) |
...
| `src/app/my-rubrics/`, `my-graded-tests/`, `my-classroom/` | Library, graded history, roster (students/classes tabs). **`my-classroom/students/[studentId]`** is the STUDENT PROFILE (student-profile PR, Part A, 2026-09-19): a quiet ledger — name, class chips, «N מבחנים בדוקים», then one row per approved test (stamped page 1 → exam event + «בדיקה אושרה ב…» → «score מתוך possible») linking to the returned exam with `?student=` context. **UI-1: nothing on it is computed across tests except the count** — no average, no trend; adding one is an open decision. The roster card is a STRETCHED LINK (UI-5: the name is the `<a>`, its `::after` covers the card, pencil/trash sit above it), the badge hides at 0 (M-A5), and the create/edit dialogs are ONE field — `StudentNameDialog`, no `הערות` anywhere (UI-4). **Delete is the PURGE** (Part B): `components/classroom/StudentDeleteDialog.tsx` asks the server's `purge-preview` which case it is (signed tests / data only / nothing — never the client's own count) and, whenever anything attributable exists, enables «מחיקה לצמיתות» only on her full name typed exactly (M-B3, `utils/purge-dialog.ts`); a grade in flight disables it (PRV-5). Copy in `copy/classroom.ts`; the exam event is spelled by `utils/exam-event.ts::examEventTitle` on BOTH the list and the profile (M-A4: stored batch name → «מבחן {id8}» → the rubric, for a batch-less test). Entry points (OD-2): roster card, the class modal's names, the returned page's title, the pile card's name (`role="link"`, inert while the card is disabled) — never the review panel. |
| `src/components/grade-review/StampedPage.tsx` | **THE stamped page 1 — one renderer, two surfaces (UI-2).** The returned page's page view (draggable stamp) and the profile's 96 px thumbnail (read-only `StampDrag`) draw the same box; the stamp's spot is size-invariant because the position is normalized and `stampBox` measures the live box (FA-3). Extracted from `ReturnedExamPreview` and proven PIXEL-IDENTICAL there before reuse (M-A7: before/after captures of the returned page at 1440, equal MD5 `6ac62c0e…` — kept local, because the page shows a real student's name and this repository is public). The returned page's way back is `utils/return-context.ts::resolveReturnContext` — TYPED context only (`?student=` a real UUID → the profile; `?batch=` → the dashboard; else המבחנים שלי), never a free-form `back=` path. OD-14: the approval moment reads «בדיקה אושרה ב{datetime}» on the profile row and the returned page's metadata line; the other «נחתם» strings are STATE labels («אושר ונחתם», «לא נחתם»), not timestamps, and were left alone. |
...
| `src/app/batches/page.tsx` | The batches **list** (P5/L1) — mini `SegmentBar` honesty bar + one action line per row, all copy from C1, and the chip from `deriveBatchStage` (the same function the detail page uses; the list used to run its own label and its own colour map). ⚠️ The list payload carries `rollup` only (no `transcriptions[]`), so the dashboard's clean\|needs-eyes split is **not derivable here**; `utils/batch-list.ts` tells a coarser truth deliberately rather than inventing one. **The section is «המבחנים שלי»** (OD-1, 2026-09-14): `מקבץ` stays the code / route / DB word and never reaches a screen, and `מבחנים בדוקים` folded in as a TAB (`components/batch/ExamsTabs.tsx`) rather than a second sidebar entry — the two lists are one section at two zoom levels, and two entries is how a teacher ends up asking which of them her work is in. `/my-graded-tests` keeps its route; nothing redirects |
...
**[§5.5, clarity pass 2026-09-14] THE AMBER DOT'S LEGEND SAYS WHAT THE DOT ACTUALLY MEANS.** The rail dot is `markerCountByScope(reviewMarkers(draft))` — SIX deterministic marker kinds, not a confidence score, which the product never renders. That is why it appears beside a FULL-MARKS question and read like a bug: the case the markers exist for is a `met` check whose cited quotation is not in the answer — invented credit, invisible in the score, and nothing else would send her there. The legend therefore reads «ויוי לא הצליחה לאמת את זה בעצמה» and never «פחות בטוחה». Beside it the rail now shows the DEDUCTION (`−d`, `utils/points-display.ts::subtractPoints`, exact via `lib/decimal.ts`, omitted at zero) — «where did he lose points» is her first read and the dots do not answer it. ⚠ **The visible approve hint is `Ctrl ↵`, not a bare Enter**: on this surface Enter opens the points field (OD-R2) and never approves, so a hint naming Enter would be false. The PR spec asked for «Enter אישור וחתימה»; the ruling wins and the hint tells the truth about it. **The product term is «המבחן החתום»** (was «המוחזר»), and the ZIP entry names moved with it — `returned_exam.ZIP_ENTRY_SUFFIX` is the one spelling, because `unique_zip_entry_names` slices it off BY LENGTH to build its dedupe stem.

**[OD-R2, owner ruling 2026-09-13] POINTS ARE TYPEABLE — on a check row and on the criterion row — and this REVERSES R-2 branch B (2026-09-01, «an override is a VERDICT, not a number»).** What R-2 protected survives untouched: there is still ONE pricer (`services/pricing.py` ↔ `lib/pricing.ts`, pinned byte-for-byte by `pricing_vectors.json` AND `pricing_vectors_typed.json` — `scripts/gen_typed_points_vectors.py`, regenerate-clean in `test_grade_review_fixtures.py`). A typed amount is a DECISION fed to that pricer beside her verdicts (`TeacherOverride.points_awarded` on a check; `GradedTestOverrides.terminal_points` on a whole terminal — a real typed map, never a sentinel check id), and both sides still price the same overlay and compare. Three rules keep glyph and number from ever disagreeing on one screen: (OD-3 b) the VERDICT IS DERIVED from the amount (`verdict_for_amount`; full → met, zero → not_met, between → partially_met, the reverse sense for a tariff) and the gate refuses any record carrying another verdict (`verdict_mismatch`); (OD-2 b) LAST TOUCH WINS between a criterion and its checks — typing on the criterion withdraws the amounts typed beneath it, deciding a check (verdict or amount) releases the criterion's pin, while a note, a dispute or ⌫ leaves the pin alone; (OD-4 a) an amount above its ceiling or off the rubric's grid is REFUSED, never snapped — live while she types (`PointsInput` + `utils/points-entry.ts`: «לא ניתן להעניק X נקודות לקריטריון עם מקסימום Y נקודות») and again at `/draft` and `/approve` (`typed_points_violations`: `out_of_bounds` / `off_grid` / `not_priceable` for note_only / `verdict_mismatch`). A typed deduction OWNS its charge group (same defect once, at HER amount; typing 0 waives it scope-wide); a criterion pin decides every check beneath it for OD-R1's «every check decided» bar. The frozen contract carries the markers (OD-5): `ContractCheck.typed_points` and `ContractTerminalOutcome.typed_points`, so the eval suite can tell «she disagreed with the tariff» from «she disagreed with the verdict». Bare Enter on the focused row opens its field (a field, not a signature — the keymap's one Enter divergence from the transcription map is now that, not «nothing»). **Two same-day amendments (owner-ruled 2026-09-13, after graded_test 3438b7a2 showed «0 / 0.5» beside a ✓ and «−0.5 / 0.5» beside a ✗):** (i) a TARIFF is a yes/no DEDUCTION — its row carries a «הורדה» chip, reads «ללא הורדה» / «−0.5» with «עד 0.5» as its ceiling, toggles ✓ ↔ ✗ only (a legacy ½ renders and prices as ✗), and has NO points field; the gate refuses a typed amount on one (`not_priceable`) and `verdict_for_amount` is kind-agnostic. (ii) THE INK GRAMMAR CHANGED: every ✓ is green, ½ yellow, ✗ red (`grade.green` / `grade.yellow` / `grade.red`), and a mark SHE made — verdict ring, figures, totals, the dispute chip, the note rail — is turquoise (`primary`). Red no longer means «the teacher» on the review surface; the stamp on the returned exam keeps its own red.
...
**Required:** `OPENAI_API_KEY`, `GOOGLE_CLOUD_PROJECT`, `DATABASE_URL` (`postgresql+asyncpg://...`). ⚠️ The DB lives in Supabase **eu-central-1** (project `ngkqawsyqqbhqthgdkpk`, migrated from ap-south-1 2026-08-23 — the Mumbai hop cost ~300 ms *per query* through the pooler and made a 10-round-trip append take ~6 s; Frankfurt took it to ~0.5 s). Cloud Run reads it from Secret Manager `database-url:latest`. The Mumbai project was RETIRED 2026-08-23 (dump archived + restore-verified at `gs://grader-vision-pdfs-0438328890/db-archive/vivi_premigration_20260822.dump` — **inside the student-data bucket**, not the nonexistent `grader-vision-pdfs` this line used to name; a pre-cleanup Frankfurt dump and a 2026-09-11 pre-launch dump + JSON export sit beside it) — `database-url:1` now points at a deleted project. ⚠ These archives are frozen copies of student rows that no purge reaches, and they are KEPT (OD-B9, ruled 2026-09-23): **PITR is off**, so they are the point-in-time restore points. A production `pg_dump` joins them before migration 032 (`docs/PURGE_CENSUS.md` §4a, §20).
...
> **THE WORKFLOW (owner-ruled 2026-09-23): implement, commit, push to `main`, deploy.** `main` has no
> branch protection and no rulesets. An agent working here is authorized to commit and push directly to
> `main`, to push the frontend deploy subtree (`frontend-deployment`), to deploy the backend to Cloud Run,
> and to apply migrations to production and to Vivi-Test — dumping production to `db-archive/` first when
> the migration alters or deletes (PITR is OFF). **No PRs, PR bodies, parity screenshots, traceability
> tables or sign-off gates unless Noam asks.** The gates that remain are the tests and the deploy checks
> below; a spec's own rulings still govern WHAT is built.

**`main` carries the full tree, and it is where work lands** (verified 2026-09-22). `git ls-tree origin/main`
