# AGENTS.md — Vivi Engineering Briefing
...
| `auth.py` / `users.py` | `/api/v0/auth`, `/api/v0/users` | auth (login/signup/**profile**/logout/refresh) · users: subject matters, the user's rubric + graded-test lists, rubric sharing. **The profile read lives at `GET /auth/me` and only there** — `users.py` carried a caller-less byte-identical duplicate at `GET /users/me`, deleted 2026-08-17 |
...
**This file (`AGENTS.md`) and `BACKLOG.md` are untracked** working-tree context — they are not in either
...
This is Vivi's entire codebase. Vivi is an AI education platform designed to assist teachers with grading tests, creating classroom materials and receiving insights about their students.  
In addition, Vivi's product roadmap is to enable personalized learning for each student based on their knowledge base which is extrapolated from their tests.
