...
export function extractQuestions(rubric: unknown): QuestionText[] {
    ...
    for (const question of questions) {
        ...
        collectSubQuestions(question.question_id, null, question.sub_questions, out);
    }
    return out;
}

/**
 * Flatten a sub-question subtree into FULL-PATH entries ("א", then "א.1").
 *
 * RECURSIVE, and the ids are paths — both halves are load-bearing. This walked
 * exactly ONE level and emitted BARE ids, so a rubric nested two deep produced
 * no entry for `א.1` AT ALL: `questionTextForScope` could not find what was
 * never emitted, and the teacher reviewed those scopes with the «השאלה»
 * disclosure empty — no question in front of her at the moment she is judging
 * whether the grade is right. Nested bagrut-style rubrics are exactly where the
 * parts diverge most, so it failed hardest where it mattered most.
 *
 * The path shape MIRRORS `gradable_compiler`'s scope ids (`sub_question_id` is
 * the path within the question), because that is what the outcomes are keyed by.
 *
 * A parent with no prose of its own is SKIPPED but still recursed into — the
 * text often lives only on the leaves (a stem with «(1) …» and «(2) …» under
 * it), and dropping the subtree because its parent was silent is how the depth-2
 * entries went missing in the first place.
 */
function collectSubQuestions(
    questionId: string,
    parentPath: string | null,
    subs: { sub_question_id?: string; text?: string; question_text?: string;
            sub_questions?: unknown }[] | null | undefined,
    out: QuestionText[],
): void {
    for (const sub of subs ?? []) {
        if (!sub?.sub_question_id) continue;
        const path = parentPath ? `${parentPath}.${sub.sub_question_id}` : sub.sub_question_id;
        const text = sub.text ?? sub.question_text;
        if (text) {
            out.push({ question_id: questionId, sub_question_id: path, text });
        }
        collectSubQuestions(
            questionId, path,
            sub.sub_questions as Parameters<typeof collectSubQuestions>[2],
            out,
        );
    }
}
