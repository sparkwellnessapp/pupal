...
# LangChain for provider-agnostic structured output
...
@trace_if_enabled(
    "rubric_service_extract_example_solutions_from_pages_trace",
    name="extract_example_solutions_from_pages",
    run_type="llm",
)
...
async def enhance_criterion_with_rules(
    ...
    The EnhancedCriterion Pydantic model validates that reduction_rules sum 
    equals total_points - if not, ValidationError is raised and can trigger retry.
    ...
    # Insert example section into prompt
    ...
    try:
        if provider == "anthropic" and ANTHROPIC_AVAILABLE:
            ...
            if api_key:
                ...
                llm = ChatOpenAI(model=model or "gpt-4o", temperature=0.0, max_tokens=2000)
        else:
            llm = ChatOpenAI(model=model or "gpt-4o", temperature=0.0, max_tokens=2000)
    except Exception as e:
        logger.warning(f"Failed to initialize LangChain LLM: {e}, falling back to legacy")
        llm = None
    ...
    for attempt in range(max_retries):
        try:
            if llm is not None:
                # Use LangChain with_structured_output for guaranteed schema compliance
                # EnhancedCriterion has built-in validator for reduction_rules sum
                structured_llm = llm.with_structured_output(EnhancedCriterion)
                ...
                # Call LLM - Pydantic validation happens automatically
                result: EnhancedCriterion = await asyncio.to_thread(
                    structured_llm.invoke,
                    messages,
                )
                
                logger.info(f"Enhanced criterion: {result.criterion_description[:50]}... ({len(result.reduction_rules)} rules)")
                
                # Convert to dict for downstream compatibility
                return {
                    "criterion_description": result.criterion_description,
                    "total_points": total_points,  # Use original, not LLM's
                    "reduction_rules": [
                        {
                            "description": rule.description,
                            "reduction_value": rule.reduction_value,
                            "is_explicit": rule.is_explicit,
                        }
                        for rule in result.reduction_rules
                    ],
                    "notes": result.notes,
                    "raw_text": criterion_text,
                ...
                # Legacy fallback if LangChain not available
                client = get_openai_client()
                response = await asyncio.to_thread(
                    client.chat.completions.create,
                    model="gpt-4o",
            ...
            # Pydantic validation failed (e.g., sum mismatch)
            last_error = e
            logger.warning(f"Schema validation failed on attempt {attempt + 1}: {e}")
            # Add error context to prompt for retry
            user_message = f"{user_message}\n\n[PREVIOUS ERROR: {str(e)}. Please fix and try again.]"
...
@trace_if_enabled(
    "rubric_service_extract_criteria_enhanced_trace",
    name="extract_criteria_enhanced",
    run_type="chain",
)
...
@trace_if_enabled(
    "rubric_service_extract_all_questions_trace",
    name="extract_all_questions",
    run_type="llm",
)
...
@trace_if_enabled(
    "rubric_service_extract_rubric_with_page_mappings_trace",
    name="extract_rubric_with_page_mappings",
    run_type="chain",
)
