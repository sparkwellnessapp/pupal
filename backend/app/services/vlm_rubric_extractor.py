...
=== אילוץ קריטי: העתקה מילה במילה ===
❌ אסור לך לשנות, לנסח מחדש, לקצר או לסכם את הטקסט!
❌ אסור לך להוסיף מילים שלא קיימות במקור!
❌ אסור לך להחסיר מילים שקיימות במקור!
✅ העתק את הטקסט בדיוק כפי שהוא מופיע בתמונה!

=== משימה ===
חלץ את טקסט השאלה המלא מהתמונה - מילה במילה!

=== מה לחלץ ===
1. כותרת השאלה (שאלה X - Y נקודות) - במילים המדויקות
2. הוראות השאלה המלאות - העתקה מדויקת
3. תת-שאלות (א, ב, ג, ד, ה, ו, ז) - אם קיימות
4. דוגמאות קוד או פלט - כולל פורמט מדויק

=== הוראות ===
1. העתק את הטקסט מילה במילה! אל תנסח מחדש!
2. שמור על סימני פיסוק, סוגריים, מספרים בדיוק כפי שהם
3. אם יש קוד לדוגמה - כלול אותו בדיוק
4. אם אין תת-שאלות, החזר רשימה ריקה
5. חלץ רק את השאלה המבוקשת

=== פורמט פלט (JSON בלבד) ===
{
  "question_number": 1,
  "total_points": 15,
  "question_text": "הטקסט המלא של השאלה - מילה במילה מהתמונה!",
  "has_sub_questions": true,
  "sub_questions": [
    {"id": "א", "text": "טקסט תת-שאלה א - העתקה מדויקת"},
    {"id": "ב", "text": "טקסט תת-שאלה ב - העתקה מדויקת"}
  ]
}"""
...
class VLMRubricExtractor:
    ...
    @trace_if_enabled(
        "vlm_rubric_extractor_extract_criteria_trace",
        name="vlm_extract_criteria",
        run_type="llm",
    )
...
אם אתה רואה טבלה עם קריטריונים, חלץ כל שורה. אם אין טבלה, החזר criteria ריק."""
    ...
    @trace_if_enabled(
        "vlm_rubric_extractor_extract_question_text_trace",
        name="vlm_extract_question",
        run_type="llm",
    )
