...
class TranscribedAnswer:
    ...
    needed_grounding_retry: bool = False
    # S11: sliding-window span-min over token logprobs for this answer's page(s).
    # None when the provider doesn't support logprobs (e.g. Anthropic/Gemini).
    min_span_logprob: Optional[float] = None
...
class OpenAIProvider(VLMProvider):
    ...
    def transcribe_images_with_logprobs(
        self,
        images_b64: List[str],
        system_prompt: str,
        user_prompt: str,
        ...
        """S11: Like transcribe_images but also returns per-token logprobs."""
        content = []
        for img_b64 in images_b64:
            content.append({
                "type": "image_url",
        ...
        return response.choices[0].message.content, token_logprobs
...
class HandwritingTranscriptionService:
    ...
    def _transcribe_page_grounded(
        ...
        # S11: use logprobs if the provider supports it (OpenAI only)
        if hasattr(self.vlm_provider, "transcribe_images_with_logprobs"):
            response, token_logprobs = self.vlm_provider.transcribe_images_with_logprobs(
                images_b64=[page_b64],
        ...
        if result and not self._verify_consistency(result):
            ...
            if result is not None:
                result["_needed_grounding_retry"] = True

        # S11: attach span-min logprob to the result dict for downstream annotation
        if result is not None:
            result["_min_span_logprob"] = self._compute_min_span_logprob(token_logprobs)

        return result
    
    @staticmethod
    def _compute_min_span_logprob(token_logprobs: List[float], window: int = 5) -> Optional[float]:
        """
        S11: Sliding-window span-min over token logprobs.
        Returns the minimum value of any window of `window` consecutive tokens.
        A span-min below the configured threshold flags the page as uncertain.
        Returns None when token_logprobs is empty (provider doesn't support logprobs).
        """
        if not token_logprobs:
            return None
        if len(token_logprobs) <= window:
            return min(token_logprobs)
        return min(
...
CRITICAL: The class name MUST be {identified_class} as you identified."""
    ...
    def _merge_grounded_results(
        ...
        for page_idx, result in enumerate(page_results):
            ...
            for ans in transcription.get("answers", []):
                ...
                if answer_text.strip():
                    needed_retry = result.get("_needed_grounding_retry", False)
                    answers_by_question[key].append({
                        "text": answer_text,
                        "confidence": confidence,
                        "page": page_idx + 1,
                        "needed_retry": needed_retry,
                        "min_span_logprob": result.get("_min_span_logprob"),  # S11
                    })
        ...
        for (q_num, sub_id), answer_parts in sorted(answers_by_question.items(), key=sort_key):
            ...
            page_numbers = sorted({part["page"] for part in answer_parts})
            needed_grounding_retry = any(part["needed_retry"] for part in answer_parts)
            # S11: worst-case span-min logprob across all pages for this answer
            span_logprobs = [p["min_span_logprob"] for p in answer_parts if p.get("min_span_logprob") is not None]
            min_span_logprob = min(span_logprobs) if span_logprobs else None
            ...
            final_answers.append(TranscribedAnswer(
                ...
                needed_grounding_retry=needed_grounding_retry,
                min_span_logprob=min_span_logprob,
            ))
