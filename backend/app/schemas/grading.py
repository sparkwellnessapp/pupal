...
class EnhancedCriterion(BaseModel):
    ...
    total_points: float = Field(..., ge=0, description="Maximum points for this criterion")
...
class LegacyExtractRubricResponse(BaseModel):
    """
    Legacy response format from PDF extract_rubric endpoint.
    
    NOTE: This is the LEGACY format used by the PDF extraction pipeline.
    The DOCX pipeline uses ontology_types.ExtractRubricResponse instead.
