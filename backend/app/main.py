...
from .api.v0 import rubric_management as rubric_management_v0
from .api.v0 import rubric_extraction_jobs as rubric_extraction_jobs_v0
...
# ORDER IS LOAD-BEARING: extraction-jobs nests under /api/v0/rubrics, which
# rubric_management also serves with a catch-all GET /{rubric_id}. Starlette is
# first-registered-wins and does NOT fall through after a UUID-parse 422, so
# this router MUST be registered BEFORE rubric_management's or every
# /api/v0/rubrics/extraction-jobs/* request 422s against {rubric_id}.
app.include_router(rubric_extraction_jobs_v0.router)
app.include_router(rubric_extraction_jobs_v0.internal_router)
app.include_router(rubric_management_v0.router)
